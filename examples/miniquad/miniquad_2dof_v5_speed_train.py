"""
miniquad_2dof_v5_speed_train.py
================================
2DOF-hip 4脚ロボット v5 速度最大化・長距離歩行訓練スクリプト。

【v4 → v5 の主な変更点: 10m 歩行を目指した速度最大化】

  問題点 (v2〜v4):
    - vx ≈ 0.010 m/s (理論最大 0.059 m/s の 17%)
    - 20s × 0.010 m/s = 0.2m → 10m には遠い
    - kp=25 で関節剛性が低く、trot_bias のトルクが地面に伝わらない

  改善 1: kp 25 → 50 (関節剛性 2倍)
    - PD コントローラの位置追従精度向上
    - trot_bias の命令どおりに足が動く → ストライドが実質的に増加
    - 理論速度への到達率 17% → 30-40% を目標

  改善 2: femur_amp 0.30 → 0.40 rad
    - ストライド長 ≈ 2 × L × sin(amp) = 2 × 0.050 × sin(0.40) = 3.89cm
    - v2 (0.30 rad): 2 × 0.050 × sin(0.30) = 2.96cm → v5 で +31%

  改善 3: forward_vel 報酬 2.0 → 8.0 (4倍)
    - 速度追求が最優先の訓練シグナルになる

  改善 4: action_rate ペナルティ緩和 -0.02 → -0.001
    - noise_std が早期崩壊しなくなり、探索を維持できる

  改善 5: エピソード長 20s → 100s
    - 長距離歩行を1エピソードで経験できる
    - 60m 歩行 (10m 目標) には vx ≈ 0.10 m/s が必要
    - まず vx ≈ 0.05 m/s を目標 → 5m / 100s

  改善 6: 全面平坦地形 (n_subterrains=8×2, 全 flat)
    - 速度学習に集中するため地形複雑化を排除
    - 16m × 4m の広大な平坦フィールド
    - 10m 歩行しても terrain 内に収まる

  改善 7: tracking_sigma 0.01 → 0.05
    - 速度誤差の勾配が広くなり、遅い速度でも正の報酬が得られる

  不変:
    - entropy_coef = 0.0 (必須: noise_std 爆発防止)
    - 摩擦ランダム化 (実機 domain gap 対応)
    - push 外乱 (頑健性維持)

【10m 達成の考え方】
  - vx=0.050 m/s を維持できれば: 0.050 × 100s = 5m / episode
  - vx=0.100 m/s なら: 0.100 × 100s = 10m / episode
  - 評価は 200s (10,000 steps) で peak_x を測定する

【実行方法】
  cd examples/miniquad
  PYTHONPATH=/home/mutsumi/rsl_rl:$PYTHONPATH python3 miniquad_2dof_v5_speed_train.py \\
    -e miniquad-2dof-v5-speed -B 16384 --max_iterations 5001 \\
    --load_run miniquad-2dof-v2-terrain --load_checkpoint 500 \\
    > /tmp/miniquad_v5_speed_train.log 2>&1 &
"""
import argparse
import os
import pickle
...
class miniquad2DOFV5SpeedEnv:
    """v5 速度最大化専用環境。
    全平坦地形 16m×4m、kp=50、100s エピソード。
    """
    ...
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg,
        ...
        # ── 全平坦地形 (16m × 4m) ──────────────────────────────────────────
        # n_subterrains=(8, 2): 8行×2列 = 8×2m + 2×2m = 16m × 4m
        # ロボットが 100s × 0.10 m/s = 10m 歩いても範囲内
        self.terrain = self.scene.add_entity(
            morph=gs.morphs.Terrain(
                ...
                randomize=False,  # 全 flat なのでランダム化不要
                name="miniquad_flat_v5",
            ),
        )
    ...
    def _reward_forward_vel(self):
        """前進速度の直接報酬 (0〜clamp_max m/s)。v5 では clamp を 0.5 m/s まで拡大。"""
        vx = self.base_lin_vel_world[:, 0]
        return torch.clamp(vx, 0.0, 0.5)   # v2: 1.0 → v5: 0.5 (クランプ小さく高速への誘導)
...
def get_cfgs():
    ...
    env_cfg = {
        ...
        # v5: 関節剛性 2倍 → trot_bias のトルクが地面に伝わる
        "kp": 50.0,
        "kd":  0.2,
        ...
        # 平坦地形の中央、16m フィールドの端から 0.5m
        "base_init_pos":  [0.5, 2.0, 0.082],
        "base_init_quat": [1.0, 0.0, 0.0, 0.0],

        # v5: 5× 長いエピソード → 長距離歩行を経験
        "episode_length_s":   100.0,
        "resampling_time_s":   10.0,  # コマンドを 10s ごとに変更
        "action_scale":        0.5,
        "simulate_action_latency": True,
        "clip_actions":       10.0,

        # v5: 1Hz (最効率) + femur_amp 拡大
        "use_trot_bias":       True,
        "trot_bias_freq":      1.0,   # 1Hz: v2 比で効率 33% (2Hz の 22% より高い)
        "trot_bias_coxa_amp":  0.25,  # 小さく (横方向の無駄を減らす)
        "trot_bias_femur_amp": 0.40,  # v2:0.30 → v5:0.40 (ストライド 31% 拡大)
    ...
    reward_cfg = {
        # tracking_sigma 拡大: 速度が低くても正の勾配が得られる
        "tracking_sigma":    0.05,
        "base_height_target": 0.082,
        "reward_scales": {
            # forward_vel を最重要報酬に (v2:2.0 → v5:8.0)
            "forward_vel":       8.0,
            # tracking_lin_vel は補助として維持
            "tracking_lin_vel":  2.0,
            "alive":             1.0,
            "lin_vel_z":        -0.2,
            "base_height":      -0.2,
            # action_rate: 最小化して探索を維持 (v2:-0.02 → v5:-0.001)
            "action_rate":      -0.001,
            "heading":          -0.1,
            "orientation":      -0.1,
        },
    ...
    command_cfg = {
        "num_commands": 3,
        # v5: コマンド上限を 0.20 m/s に (v2:0.15 → v5:0.20)
...
def main():
    ...
    parser.add_argument("-e", "--exp_name",     type=str, default="miniquad-2dof-v5-speed")
